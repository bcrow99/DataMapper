...
public class GetVarianceImage
{
	...
	{	 
        ...
        // This is meters and we want quarter meters, half the resolution of our sensors.
        xdim *= resolution;
        ydim *= resolution;
        ...
        for(int i = 0; i < ydim; i++)
        {
        	for(int j = 0; j < xdim; j++)
        	{
        		...
        		cell_intensity[i][j] = minimum_intensity + intensity_range / 2;
        	}
        }
        ...
        {  
        	...
        	for(int j = 0; j < xdim; j++)
        	{
        	    ...
        	    {  
        	    	...
        	    	//would work and keep things simple and organized--the corner lists would always contain
        	    	//the closest points and assigning them is easy but it creates problems somehow that I don't understand.
        	    	//For whatever reasons, this works better--assigning points to all the 
        	    	//different lists.
	    ...
	    int gray[]             = new int[xdim * ydim];
	    int variance[]         = new int[xdim * ydim];
        ...
        DataMapper.getPixelVariance(gray, xdim, ydim, variance);
        BufferedImage variance_image = new BufferedImage(xdim, ydim, BufferedImage.TYPE_INT_RGB);
        for(int i = 0; i < ydim; i++)
        {
        	for(int j = 0; j < xdim; j++)
        	{
        		int k = i * xdim + j;
                int rgb_value = ((variance[k] & 0x0ff)<<16)|((variance[k] & 0x0ff)<<8)|(variance[k] & 0x0ff);
    	        variance_image.setRGB(j, i, rgb_value);  
        	}
        }
        try 
        {  
            ImageIO.write(variance_image, "jpg", new File("C:/Users/Brian Crowley/Desktop/delta.jpg")); 
        } 
        catch(IOException e) 
        {  
            e.printStackTrace(); 
