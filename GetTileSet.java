...
public class GetTileSet
{
	public static void main(String[] args) throws IOException, FileNotFoundException
	{
		// Setting these values so the compiler won't complain.
		int tile_id = 0;
		int resolution = 4;
		int tile_xmin = 0;
		int tile_xmax = 6;
		...
		if (args.length == 3)
		{
			try
			{
				tile_xdim = Integer.parseInt(args[0]);
				tile_ydim = Integer.parseInt(args[1]);
		...
		{
			System.out.println("Usage: GetWeightedTileSet <xdim> <ydim> <resolution>");
			System.exit(0);
		}
		int number_of_tiles = global_xdim / tile_xdim * global_ydim / tile_ydim;
		System.out.println("The number of tiles is " + number_of_tiles);
		...
		tile_xmax = tile_xdim;
		tile_ymin = 15;
		tile_ymax = 15 + tile_ydim;

		while (tile_id < number_of_tiles)
		{
			System.out.println("Tile id is " + tile_id);
			System.out.println("Tile x min is " + tile_xmin);
			System.out.println("Tile y min is " + tile_ymin);
			ArrayList sample_list = new ArrayList();
			ArrayList complete_list_copy = new ArrayList();
			for (int i = 0; i < complete_sample_list.size(); i++)
			{
				Sample current_sample = (Sample) complete_sample_list.get(i);
				Sample sample_copy = new Sample(current_sample.x, current_sample.y, current_sample.intensity);
				complete_list_copy.add(i, sample_copy);
			}

			String file_string = new String("C:/Users/Brian Crowley/Desktop/images/400-tile/WTile" + tile_id + ".png");
			Sample previous_sample, current_sample, next_sample;

			int column = tile_xmin / 2;
			if (column > 0)
				column--;
			int start_line = column;
			column = tile_xmax / 2;
			if (column > 28)
				column = 29;
			else
				column++;
			int stop_line = column;

			System.out.println("Start line is " + start_line);
			System.out.println("Stop line is " + stop_line);
			...
			for (int i = start_line; i <= stop_line; i++)
			{
				...
				{
					Sample sample = (Sample) complete_list_copy.get(j);
			...
			System.out.println("Sample list size is " + sample_list_size);
			...
			double x_origin = tile_xmin;
			double y_origin = tile_ymin;
			...
			int xdim = tile_xdim * resolution;
			int ydim = tile_ydim * resolution;
			...
			for (int i = 0; i < ydim; i++)
			{
				...
				for (int j = 0; j < xdim; j++)
				{
					...
					ArrayList neighbor_list = DataMapper.getNeighborList(j, i, xdim, ydim, close_data_array,
							isPopulated);
					for (int k = 0; k < 8; k++)
					{
						...
						if (list.size() != 0)
						{
							...
							for (int m = 0; m < list.size(); m++)
							{
								...
								{
								case 0:
									southwest_list.add(sample);
									break;
								case 1:
									south_list.add(sample);
									break;
								case 2:
									southeast_list.add(sample);
									break;
								case 3:
									west_list.add(sample);
									break;
								case 4:
									east_list.add(sample);
									break;
								case 5:
									northwest_list.add(sample);
									break;
								case 6:
									north_list.add(sample);
									break;
								case 7:
									northeast_list.add(sample);
								}
							}
						}
					}
					...
					for (int m = 0; m < 8; m++)
					{
						...
						if (sorted_list.size() != 0)
						{
							...
							// Probably happens when the drone is flying so slow that
							...
							// to calculate incremental values for duplicate data.

							// For now we'll go through the list and
							...
							// We'll use the minimum possible floating point value.
			...
			for (int i = 0; i < ydim; i++)
			{
				...
				for (int j = 0; j < xdim; j++)
				{
					...
					} else if (number_of_quadrants == 2)
					{
						...
						isInterpolated[i][j] = true;
					} else if (neighborPopulated[0] && neighborPopulated[2] && neighborPopulated[7]
							&& neighborPopulated[5])
					{
						...
						cell_intensity[i][j] = DataMapper.getLinearInterpolation(origin, first_sample, second_sample,
								third_sample, fourth_sample);
						isInterpolated[i][j] = true;
						number_of_linear_interpolations++;
					} else if (neighborPopulated[1] && neighborPopulated[4] && neighborPopulated[6]
							&& neighborPopulated[3])
					{
					...
					} else if (number_of_quadrants > 2)
					{
						...
						actual_set_table = QuadrantMapper.getActualTriangleSetTable(possible_set_list, neighbor_list,
								origin);
						if (!actual_set_table.isEmpty())
						{
							...
							cell_intensity[i][j] = DataMapper.getLinearInterpolation(origin, first_sample,
			...
			// System.out.println("The number of interpolated cells was " +
			// number_of_interpolated_cells);
			// System.out.println("The number of linear interpolations was " +
			// number_of_linear_interpolations);
			// System.out.println("The number of bisecting averages was " +
			// number_of_bisecting_averages);
			// System.out.println("The number of weighted averages was " +
			// number_of_weighted_averages);
			// System.out.println("The number of nearest neighbors was " +
			// number_of_nearest_neighbors);
			...
			if (number_of_cells != number_of_interpolated_cells)
			{
				...
				System.out.println(
						"The number of cells assigned by dilation was " + number_of_cells_assigned_by_dilation);
			...
			for (int i = 0; i < ydim; i++)
			{
				for (int j = 0; j < xdim; j++)
				{
					...
					int m = ydim - 1 - i;
					if (wasDilated)
						current_value = dilated_image[m][j];
					else
						current_value = cell_intensity[m][j];
					current_value -= interpolated_min;
					/*
					 * This preserves precision for image processing by keeping what looks like
					 * 14-bit data. current_value *= 100; gray_value[k] = (int)current_value;
					 */

					// This gives us something we can easily look at on a 3 x 8 bit display.
					// The division and multiplication reduce the accuracy even while it increases
					// the range.
			...
			try (PrintWriter output = new PrintWriter(filename))
			{
				...
				{
					String xstring = String.format("%.2f", xposition);
					...
					for (int j = 0; j < ydim; j++)
					{
						...
						String ystring = String.format("%.2f", yposition);
