...
public class GetTriangle
{
	...
	{	 
        ...
        */
        ...
        // Quadrilaterals do not appear to be smoother than triangles, although I have some bugs in the implementation.
        // Taking a whack at improving bisecting average to see if it helps.  
        	...
        	{ 
        	   ...
        	   default: possible_set_list = QuadrantMapper.getPossibleTriangleSets(neighbor_index_list);
        	            ...
        	            else
        	            {
    	            	    ...
    	            	    Point2D.Double reference_point = new Point2D.Double(x3,y3);
    	            	    double         reference_value = third_sample.intensity;
    	            	    
    	            	    
    	            	    // Find out if we have true bisectors and make adjustment if so since the delta function is straightforwad. 
    	            	    // If both the origin and the reference point form true bisectors the delta function is simply a negative or positive
    	            	    // change in intensity directly proportional to the distance, depending on whether the reference point and the point
    	            	    // of interest are on the the same side of the line.
    	            	    // It might be possible to work something out in the other cases--such as extending the line
    	            	    // to make sure there are true bisectors and extrapolating values for the new endpoints.
    	            	    
    	            	    boolean hasBisectors = true;
    	            	    double endpoint_distance1, endpoint_distance2;
    	            	    // Check if returns same value as getLength.
    	            	    endpoint_distance1 = DataMapper.getDistance(xcenter, ycenter, x1, y1);
    	            	    endpoint_distance2 = DataMapper.getDistance(xcenter, ycenter, x2, y2);
    	            	    double origin_distance = reference_line.ptSegDist(xcenter, ycenter);
    	            	    if(endpoint_distance1 == origin_distance || endpoint_distance2 == origin_distance)
    	            	    	hasBisectors = false;
    	            	    endpoint_distance1 = DataMapper.getDistance(x3, y3, x1, y1);
    	            	    endpoint_distance2 = DataMapper.getDistance(x3, y3, x2, y2);
    	            	    double reference_distance = reference_line.ptSegDist(xcenter, ycenter);
    	            	    if(endpoint_distance1 == reference_distance || endpoint_distance2 == reference_distance)
    	            	    	hasBisectors = false;
    	            	    if(hasBisectors)
    	            	    {
    	            	    	System.out.println("Got here.");
    	            	        double         bisecting_value  = DataMapper.getBisectingAverage(first_sample, second_sample, reference_point);
    	            	        double         delta            = reference_value - bisecting_value;
    	            	        double         delta_factor     = reference_value / reference_distance;
    	            	        double         unadjusted_value = DataMapper.getBisectingAverage(first_sample, second_sample, origin);
    	            	        double         adjusted_value   = unadjusted_value;
    	            	        
    	            	        if(DataMapper.areSameSide(reference_line, x3, y3, xcenter, ycenter))
    	            	        	adjusted_value -= delta_factor * origin_distance;
    	            	        else
    	            	        	adjusted_value += delta_factor * origin_distance;
    	            	        // Clip the value. 
    	            	        if(adjusted_value > maximum_intensity)
    	            	        	adjusted_value = maximum_intensity;
    	            	        else if(adjusted_value < minimum_intensity)
    	            	        	adjusted_value = minimum_intensity;
       	            	        cell_intensity[i][j] = DataMapper.getBisectingAverage(first_sample, second_sample, origin);
    	            	        //cell_intensity[i][j] = adjusted_value;
